...
async function createRetargeting() {
  try {
    ...
    const audience = await ms.apiPost(`/${accountId}/customaudiences`, {
      ...
      subtype: 'WEBSITE',
      retention_days: 30,
      pixel_id: process.env.META_PIXEL_ID || '2089219804702392',
      rule: JSON.stringify({
        and: [
          {
            event: 'PageView',
    ...
    const adSet = await ms.apiPost(`/${accountId}/adsets`, {
      ...
      status: 'PAUSED',
      daily_budget: 500 * 100, // ₹500
      billing_event: 'IMPRESSIONS',
      optimization_goal: 'REACH', // For small retargeting audiences, Reach or Landing Page Views is often better than Sales
