...
async function optimizeMetaTargeting() {
  ...
  for (const update of updates) {
    try {
      ...
      // Fetch current targeting directly for this ID
      const current = await ms.api(`/${update.id}`, { fields: 'targeting' });
      
      if (!current || !current.targeting) {
        console.warn(`AdSet ${update.id} targeting not found.`);
        continue;
      }
